...
void Gnss::gpsSvStatusCb(GpsSvStatus* svInfo) {
    ...
    uint32_t gloUsedInFixMask = *(&(svInfo->used_in_fix_mask) + 1); // this would be the next int in the HaxxSvStatus struct
    /*
    ...
    for (size_t i = 0; i < svStatus.numSvs; i++) {
        ...
        if (info.constellation == GnssConstellationType::GPS || info.constellation == GnssConstellationType::GLONASS) {
            ...
            if ((info.constellation == GnssConstellationType::GPS && (usedInFixMask & svidMask) != 0) ||
                (info.constellation == GnssConstellationType::GLONASS && (gloUsedInFixMask & svidMask) != 0)) {
